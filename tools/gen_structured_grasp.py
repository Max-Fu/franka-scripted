...
class FrankaCubeGrasp(FrankaBase):
    def __init__(
        self, 
        ...
        # set the canonical rotation matrix
        ...
        self.gen_grasps(z_height=self.z_height)
        
        self.upper_disk_height, self.upper_disk_height_rad, self.upper_disk_rad = 0.25, 0.05, 0.1
        self.lower_disk_height, self.lower_disk_height_rad, self.lower_disk_rad = 0.1, 0.04, 0.02
    ...
    def gen_grasps(self, z_height=0.015):
        self.grasps = []
        ...
        for grasp_idx in range(self.num_grasps):
            ...
            candidate_pose = self.gen_random_pose(all_other_poses, z_rotation_range=(-np.pi/4, np.pi/4), z_height=z_height)
            # we maintain two representations: the grasps in execution order and another in the per object order
            self.grasps.append(
                {
                    "object": cur_object_name,
                    "pick": self.object_poses[cur_object_name][-1], 
                    "place": candidate_pose,
                }
            )
            self.object_poses[cur_object_name].append(candidate_pose)
        return self.grasps
    ...
    def step(self, action):
        object_name, grasp_pose, next_grasp_pose = action["object"], action["pick"], action["place"]
        ...
        info = {
            ...
            "task": "pick",
            "object": object_name,
            "grasp_pose": grasp_pose.matrix.tolist(),
        ...
        self.goto_pose(next_grasp_pose, z_offset=0.15)
        self.goto_pose(next_grasp_pose)
...
if __name__ == "__main__":
    ...
    data_collection = FrankaCubeGrasp(
